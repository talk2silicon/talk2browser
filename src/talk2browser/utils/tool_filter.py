...
def filter_tools(
    user_input: str, tools: List[Dict[str, Any]], state: Dict[str, Any], top_k: int = 5
) -> List[Dict[str, Any]]:
    """
    ...
    try:
        ...
        for tool in tools:
            ...
            state_context = (
                f"page_url: {state.get('url', '')} page_title: {state.get('title', '')}"
            )
        ...
        if logger.isEnabledFor(logging.DEBUG):
            ...
            for i, result in enumerate(sorted_results[:top_k], 1):
                logger.debug(
                    f"{i}. {result['name']}: {result['score']:.3f} - {result['description']}"
                )
