...
class SensitiveDataService:
    ...
    def __init__(self, secrets: Optional[Dict[str, str]] = None):
        ...
        logger.debug(
            f"SensitiveDataService initialized with keys: {list(self._secrets.keys())}"
        )
    ...
    def get_placeholder_for_value(cls, value: str) -> Optional[str]:
        ...
        for key, val in cls._instance._secrets.items():
            if isinstance(val, str) and val.strip() == norm_value:
                logger.debug(
                    f"[SensitiveDataService] Found secret match for value '{value}' with key '{key}', returning placeholder '${{{key}}}'"
                )
                return f"${{{key}}}"
            else:
                logger.debug(
                    f"[SensitiveDataService] No match for value '{value}' with secret key '{key}': '{val}'"
                )
        ...
        for key in os.environ:
            ...
            if isinstance(env_val, str) and env_val.strip() == norm_value:
                logger.debug(
                    f"[SensitiveDataService] Found env match for value '{value}' with key '{key}', returning placeholder '${{{key}}}'"
                )
                return f"${{{key}}}"
            else:
                logger.debug(
                    f"[SensitiveDataService] No match for value '{value}' with env key '{key}': '{env_val}'"
                )
        logger.warning(
            f"[SensitiveDataService] No placeholder found for value '{value}'. Checked secrets and env vars."
        )
        return None
    ...
    def configure(cls, secrets: Optional[Dict[str, str]] = None) -> None:
        ...
        logger.debug(
            f"SensitiveDataService configured with keys: {list((secrets or {}).keys())}"
        )
        logger.debug(
            f"[SensitiveDataService.configure] id={id(cls._instance)} keys={list(cls._instance._secrets.keys())}"
        )
    ...
    def get(cls, key: str, default: Optional[str] = None) -> Optional[str]:
        ...
        if cls._instance is None:
            logger.warning(
                "SensitiveDataService.get() called before configuration. Returning None."
            )
            return os.environ.get(key, default)
        logger.debug(
            f"[SensitiveDataService.get] id={id(cls._instance)} keys={list(cls._instance._secrets.keys())} lookup={key}"
        )
