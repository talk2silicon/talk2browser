...
class ActionService:
                ...
                try:
                    ...
                    if not hash_val:
                        lookup_xpath = (
                            ...
                            if selector
                            and (
                                selector.startswith("/") or selector.startswith("html/")
                            )
                            else xpath
                        )
                        if lookup_xpath and hasattr(
                            self.dom_service, "_interactive_elements"
                        ):
                    ...
                    if hash_val:
                        logger.debug(
                            f"[ActionService] Resolved hash for action: {action_type} -> {hash_val}"
                        )
                    else:
                        logger.warning(
                            f"[ActionService] Could not resolve hash for action: {action_type} (selector={selector}, xpath={xpath})"
                        )
                except Exception as e:
                    logger.error(f"[ActionService] Exception while resolving hash: {e}")
            else:
                logger.warning(
                    "[ActionService] dom_service not set, cannot resolve hash for agent action."
                )
            ...
            for key in ["text", "value"]:
                if key in args:
                    ...
                    if hasattr(SensitiveDataService, "get_placeholder_for_value"):
                        placeholder = SensitiveDataService.get_placeholder_for_value(
                            real_value
                        )
                    if placeholder:
                        logger.debug(
                            f"[ActionService] Masking sensitive value for {action_type}: {key}={real_value} -> {placeholder}"
                        )
                        args[key] = placeholder
        # Remove hash from args before saving JSON
        if "hash" in args:
            logger.debug(
                f"[ActionService] Removing hash from action args before saving: {args['hash']}"
            )
            args.pop("hash")
        action["args"] = args
        self.agent_actions.append(action)
        logger.debug(
            f"[ActionService] Agent actions now has {len(self.agent_actions)} actions: {self.agent_actions}"
        )
    ...
    def record_screenshot(self, screenshot_path: str) -> None:
        ...
        logger = logging.getLogger(__name__)
        action = {"type": "screenshot", "args": {"path": screenshot_path}}
    ...
    async def expose_mode_change_handler(self, page: typing.Any) -> None:
        ...
        """
        logger.info(
            "Exposing notifyPythonOfModeChange to Playwright page (via ActionService)."
        )
            ...
            while self._manual_mode:
                try:
                    await asyncio.wait_for(
                        self._resume_event.wait(), timeout=timeout_seconds
                    )
    ...
    def _perform_realtime_merge(self) -> None:
        ...
        manual_map = {
            self._compute_action_key(a): a
            for a in self.manual_actions
            if self._compute_action_key(a)
        }
        merged = []
        ...
        for agent_action in self.agent_actions:
            ...
            if key and element_map and isinstance(key, str) and key.startswith("#"):
                if key not in element_map:
                    logger.warning(
                        f"Agent action key {key} not found in DOMService element map.\nAction: {agent_action}\nElement map keys: {list(element_map.keys())[:10]} ..."
                    )
        ...
        for key, manual_action in manual_map.items():
            if key not in used_manual_keys:
                if key and element_map and isinstance(key, str) and key.startswith("#"):
                    if key not in element_map:
                        logger.warning(
                            f"Manual action key {key} not found in DOMService element map.\nAction: {manual_action}\nElement map keys: {list(element_map.keys())[:10]} ..."
                        )
    ...
    def record_manual_action(self, action: dict) -> None:
        ...
        logger.debug(
            f"[ActionService] Manual action callstack:\n{traceback.format_stack()}"
        )
        if not self.is_manual_mode:
            logger.warning(
                "[ActionService] Manual action recorded while NOT in manual mode! This may indicate a bug."
            )
        self.manual_actions.append(action)
        logger.debug(
            f"[ActionService] Manual actions now has {len(self.manual_actions)} actions: {self.manual_actions}"
        )
        self._perform_realtime_merge()
    ...
    def pop_new_manual_actions(self) -> List[Dict[str, Any]]:
        ...
        logger.debug(
            f"[ActionService] Popping {len(self.manual_actions)} new manual actions for agent context injection."
        )
    ...
    def save_merged_actions_with_prompt(
        self, task: str, output_dir: str = "./generated/actions"
    ) -> str:
        """
        ...
        prefix = (
            "_".join(re.sub(r"[^a-zA-Z0-9]+", " ", task).lower().split()[:5])
            if task
            else "scenario"
        )
