...
class ScriptGenerationService:
    ...
    async def generate_playwright_script(
        ...
        for i, action in enumerate(actions, 1):
            ...
            if action_type == "navigate":
                formatted_actions.append(
                    f"{i}. navigate to {args.get('url', '<missing url>')}"
                )
            elif action_type == "click":
                formatted_actions.append(
                    f"{i}. click {args.get('selector', '<missing selector>')}"
                )
            elif action_type == "fill":
                formatted_actions.append(
                    f"{i}. fill {args.get('selector', '<missing selector>')} with {args.get('text', '<missing value>')}"
                )
            else:
        ...
        if language == "typescript":
            ...
            ext = "py"
        self.logger.info(
            f"[ScriptGen] Calling LLM to generate Playwright {language} script for: {task}"
        )
        ...
        try:
            ...
            if language == "typescript":
                ...
                if (
                    "import { test" not in script
                    or "from '@playwright/test'" not in script
                ) and (
                    "import { test" not in script
                    or 'from "@playwright/test"' not in script
                ):
                    self.logger.warning(
                        "[ScriptGen] LLM output for TypeScript does not look like TypeScript. Output head:\n"
                        + script[:200]
                    )
            ...
            self.logger.info(
                f"Playwright script generated at {output_path} via save_text_to_file"
            )
    ...
    async def generate_cypress_script(
        self,
        actions: List[Dict[str, Any]],
        task: str,
        output_path: Optional[str] = None,
        ...
        self.logger.info(
            f"[ScriptGen] Calling LLM to generate Cypress script for: {task}"
        )
        self.logger.debug(f"[ScriptGen] LLM prompt: {prompt}")
        try:
            response = await self.llm.ainvoke(prompt)
            ...
            save_text_to_file(output_path, script)
            self.logger.info(
                f"Cypress script generated at {output_path} via save_text_to_file"
            )
            return str(Path(output_path).absolute())
        except Exception as e:
    ...
    async def generate_selenium_script(
        ...
        self.logger.info(
            f"[ScriptGen] Calling LLM to generate Selenium script for: {task}"
        )
        self.logger.debug(f"[ScriptGen] LLM prompt: {prompt}")
        try:
            response = await self.llm.ainvoke(prompt)
            ...
            save_text_to_file(output_path, script)
            self.logger.info(
                f"Selenium script generated at {output_path} via save_text_to_file"
            )
            return str(Path(output_path).absolute())
        except Exception as e:
