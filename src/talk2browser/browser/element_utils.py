...
async def find_interactive_elements(
    page: Page, max_elements: int = 100
) -> List[Dict[str, Any]]:
...
async def _get_element_properties(
    handle: ElementHandle, selector: str
    ...
    try:
        ...
        try:
            ...
            if (
                ...
                or data_action is not None
                or data_link is not None
                or data_click is not None
            ):
        ...
        if (
            not text_content
            and tag_name not in ("input", "button", "select", "textarea")
            and not is_likely_clickable
        ):
            return None
