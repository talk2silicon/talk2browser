    ...
    async def show_manual_mode_timeout_popup(self):
        ...
        await self.page.evaluate(
            ...
            if (!document.getElementById('manualTimeoutPopup')) {
              ...
              popup.innerHTML = '<div style="padding:24px;background:rgba(34,40,49,0.95);color:white;border-radius:12px;\
box-shadow:0 8px 32px rgba(31,38,135,0.37);text-align:center;font-size:1.1em;">Manual mode has been active for a while.<br><br>\
<button id="resumeAgentBtn" style="margin:8px;padding:8px 16px;background:#2196f3;color:white;border:none;border-radius:6px;">Resume Agent</button>\
<button id="continueManualBtn" style="margin:8px;padding:8px 16px;background:#757575;color:white;border:none;border-radius:6px;">Continue Manual</button></div>';
        ...
        try:
            ...
            for elem_id, elem_data in dom_tree["map"].items():
                if elem_data.get("isInteractive"):
                    ...
                    # Maintain history map for legacy compatibility
                    self._element_history_map[element.element_hash] = element
    ...
    async def click_element_by_hash(self, element_hash: str) -> bool:
        ...
        if not element:
            logger.warning(
                f"Element with hash {h} not found in history map (size: {len(self._element_history_map)})"
            )
            await self.get_interactive_elements()
            element = self._element_history_map.get(h)
            if not element:
    ...
    async def get_dom_tree(
        ...
        """
        logger.debug(
            "Getting DOM tree with highlight=%s, focus=%d",
            highlight_elements,
            focus_element,
        )
        ...
        try:
            ...
            raise ValueError(
                "JavaScript evaluation failed - page may be in invalid state"
            ) from e
    ...
    async def _highlight_elements(self, elements: List[DOMElement]):
        ...
        elements_data = [
            {"xpath": elem.xpath, "tag_name": elem.tag_name, "highlight_index": i}
            for i, elem in enumerate(elements)
        ]

        logger.debug(
            f"Calling window.highlightElements with {len(elements_data)} elements."
        )
        try:
            await self.page.evaluate(
    ...
    async def format_elements(self) -> Tuple[str, Dict[str, str]]:
    ...
    async def wait_for_calendar_popup(
        self, selector: str = "", timeout: int = 4000
    ) -> bool:
        """
        ...
        if not any(k in selector.lower() for k in calendar_keywords):
            logger.info(
                f"[DOMService] Selector '{selector}' does not match calendar keywords."
            )
            return False
        popup_selectors = [
            '[role="dialog"]',
        ...
        for popup_selector in popup_selectors:
            try:
                result = await wait_for_selector(
                    popup_selector, state="visible", timeout=timeout
                )
                if "Waited" in result:
                    logger.info(
                        f"[DOMService] Calendar popup detected with selector: {popup_selector}"
                    )
                    found_popup = True
                    break
            except Exception as e:
        ...
        if not found_popup:
            logger.warning(
                f"[DOMService] No calendar popup appeared after click on {selector}"
            )
        return found_popup
