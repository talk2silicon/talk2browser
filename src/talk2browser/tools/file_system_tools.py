...
def compress_image_to_size_limit(
    ...
    with Image.open(path) as img:
        ...
        while compressed_img.tell() > max_size and quality > min_quality:
            quality -= 10
            logger.debug(
                f"[FileSystemTools] Reducing image quality to {quality} to meet size limit"
            )
            compressed_img = io.BytesIO()
            img.save(compressed_img, format="JPEG", quality=quality, optimize=True)
        ...
        if compressed_img.tell() > max_size:
            ...
            while compressed_img.tell() > max_size and resize_factor_current > 0.3:
                ...
                compressed_img = io.BytesIO()
                resized_img.save(
                    compressed_img, format="JPEG", quality=quality, optimize=True
                )
                if compressed_img.tell() > max_size:
                ...
                if new_width < 100 or new_height < 100:
                    logger.warning(
                        "[FileSystemTools] Image too small after resizing; cannot compress further."
                    )
                    break
                logger.debug(
                    f"[FileSystemTools] Resized image to {resize_factor_current:.2f}x original size to meet size limit"
                )
...
def list_files_in_folder(
    folder_path: str, extension_filter: Optional[str] = None
) -> List[str]:
    """
    ...
    logger.info(
        f"Found {len(files)} files in {folder_path} with filter {extension_filter}"
    )
