...
async def list_suggestions(container_selector: str, option_selector: str = None, **kwargs) -> str:
    ...
    try:
        from .browser_tools import resolve_selector
    ...
    async def _generate():
        try:
            async with async_playwright() as p:
                ...
                logger.info(f"[generate_pdf_from_html] Generating enhanced PDF: {output_path}")
                pdf_options = {
                    'path': output_path,
                ...
                pdf_options['margin'] = {k: str(v) if not isinstance(v, str) else v for k, v in pdf_options['margin'].items()}
                logger.info(f"[generate_pdf_from_html] Calling Playwright page.pdf with options: {pdf_options}")
                await page.pdf(**pdf_options)
                logger.info(f"[generate_pdf_from_html] Playwright PDF generated at {output_path}")
                await browser.close()
                ...
                if screenshot_paths:
                    try:
                        ...
                        for idx, img_path in enumerate(screenshot_paths):
                            ...
                            temp_img_pdf = str(pdf_dir / f"screenshot_{idx}_{timestamp}.pdf")
                        ...
                        for temp_pdf in temp_pdf_paths:
                            ...
                            for page in img_reader.pages:
                                writer.add_page(page)
                            logger.info(f"[generate_pdf_from_html] Appending screenshot PDF: {temp_pdf}")
                        with open(output_path, "wb") as f_out:
                            writer.write(f_out)
                        for temp_pdf in temp_pdf_paths:
                            os.remove(temp_pdf)
                            logger.info(f"[generate_pdf_from_html] Deleted temp screenshot PDF: {temp_pdf}")
                    except Exception as e:
                        logger.error(f"[generate_pdf_from_html] Failed to append screenshots to PDF: {e}")
                pdf_path = Path(output_path)
                if pdf_path.exists():
                    file_size = pdf_path.stat().st_size
                    logger.info(f"Enhanced PDF generated successfully: {output_path} ({file_size:,} bytes)")
                else:
                ...
                ActionService.get_instance().record_agent_action(action_data)
                logger.info(f"[generate_pdf_from_html] Enhanced PDF generation completed with options: {merged_options}")
                logger.info(f"[generate_pdf_from_html] Returning output path: {output_path}")
                return output_path
        except Exception as e:
            logger.error(f"[generate_pdf_from_html] PDF generation failed: {e}", exc_info=True)
            raise
...
@tool
async def extract_structured_data(extract_links: bool = False, include_hidden: bool = False, **kwargs):
    """
    ...
    import asyncio
    logger = logging.getLogger(__name__)
    logger.info(f"TOOL CALL: extract_structured_data(extract_links={extract_links}, include_hidden={include_hidden})")
    try:
        browser_page = PageManager.get_instance().get_current_page()
        if not browser_page:
            logger.error("No active browser page found in PageManager.")
        ...
        logger.info(f"[extract_structured_data] Page load state: networkidle reached")
        ...
        try:
            ...
            try:
                ...
                logger.info(f"[extract_structured_data] Raw HTML extracted, length: {len(page_html_result)} characters")
                logger.info(f"[extract_structured_data] HTML preview (first 500 chars): {page_html_result[:500]}")
            except asyncio.TimeoutError:
                logger.error("Timeout while extracting page HTML content (30s exceeded)")
                return "Error: Timeout while extracting page HTML content (30 seconds exceeded)"
            logger.info(f"[extract_structured_data] Page content extraction took {time.time() - start_time:.2f}s")
            
            page_html = page_html_result
            markdownify_func = lambda html: markdownify.markdownify(html, strip=strip)
            content = None
            try:
                md_start = time.time()
                content = await asyncio.wait_for(loop.run_in_executor(None, markdownify_func, page_html), timeout=30.0)
                logger.info(f"[extract_structured_data] Markdownify took {time.time() - md_start:.2f}s")
                logger.info(f"[extract_structured_data] Markdown content length: {len(content)} characters")
                logger.info(f"[extract_structured_data] Markdown preview (first 1000 chars): {content[:1000]}")
            except asyncio.TimeoutError:
                logger.warning("Timeout during markdownify conversion (30s exceeded). Returning partial HTML content as fallback.")
                # Return partial HTML as fallback, truncated if needed
                fallback_html = page_html[:15000] + "\n... [TRUNCATED DUE TO TIMEOUT] ...\n" if page_html else ""
                logger.info(f"[extract_structured_data] Returning fallback HTML, length: {len(fallback_html)}")
            ...
            for iframe in page.frames:
                try:
                    await asyncio.wait_for(iframe.wait_for_load_state(timeout=1000), timeout=2.0)
                except Exception:
                    pass
                if iframe.url != page.url and not iframe.url.startswith('data:') and not iframe.url.startswith('about:'):
                    ...
                    content += f'\n\nIFRAME {iframe.url}:\n'
                    try:
                        iframe_html = await asyncio.wait_for(iframe.content(), timeout=5.0)
                        iframe_markdown = await asyncio.wait_for(loop.run_in_executor(None, markdownify_func, iframe_html), timeout=5.0)
                        logger.info(f"[extract_structured_data] Iframe {iframe_count} content extracted, length: {len(iframe_markdown)}")
                    except Exception as e:
                        logger.warning(f"[extract_structured_data] Failed to extract iframe {iframe_count}: {str(e)}")
                        iframe_markdown = ''
                    content += iframe_markdown
            
            logger.info(f"[extract_structured_data] Total iframes processed: {iframe_count}")
            
            cleaned_content = re.sub(r'\n+', '\n', content)
            logger.info(f"[extract_structured_data] After cleaning, content length: {len(cleaned_content)} characters")
            
            max_chars = 30000
            if len(cleaned_content) > max_chars:
                logger.info(f'Content is too long, removing middle {len(cleaned_content) - max_chars} characters')
                cleaned_content = (
                    cleaned_content[: max_chars // 2]
                    + '\n... left out the middle because it was too long ...\n'
                    + cleaned_content[-max_chars // 2 :]
                )
                logger.info(f"[extract_structured_data] After truncation, content length: {len(cleaned_content)} characters")
            
            logger.info(f"[extract_structured_data] Final content length: {len(cleaned_content)} characters")
            logger.info(f"[extract_structured_data] Final content preview (first 2000 chars):\n{cleaned_content[:2000]}")
            logger.info(f"[extract_structured_data] Final content preview (last 1000 chars):\n{cleaned_content[-1000:]}")
            ...
            logger.info(f"[extract_structured_data] Successfully returning {len(cleaned_content)} characters of extracted content")
            return cleaned_content
        except ImportError:
            logger.warning("markdownify not available, using plain text extraction")
            # Fallback to plain text extraction
            content = await page.inner_text('body')
            logger.info(f"[extract_structured_data] Fallback plain text extraction, length: {len(content)} characters")
            logger.info(f"[extract_structured_data] Plain text preview (first 1000 chars): {content[:1000]}")
            return content
    except Exception as e:
